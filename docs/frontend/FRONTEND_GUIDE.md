...
### 10.1 Tailwind Only — Everything Global

- **No inline styles** (`style={{ color: ... }}`)
- **No hardcoded color values** (`text-[#3b82f6]`, `bg-[#f5f5f5]`)
- **No hardcoded font sizes in arbitrary values** (`text-[13px]`)
- **All colors, fonts, spacing tokens must come from `tailwind.config.js`**
...
module.exports = {
  theme: {
    extend: {
      colors: {
        // All referenced from CSS variables — defined once in globals.css
...
```css
/* globals.css — define all tokens here, one place */
:root {
...
Usage — only Tailwind tokens, never raw values:

```tsx
// ❌ Wrong — hardcoded values
<div style={{ color: '#2563eb' }}>Headline</div>
<p className="text-[#666] text-[13px]">Body text</p>

// ✅ Correct — all from global tokens
<div className="text-primary">Headline</div>
<p className="text-muted-foreground text-sm">Body text</p>
```

---

### 10.2 Text Size Scale — No Arbitrary Sizes
...
### 10.3 Responsive Height — Use `vh`, Not `px`
...
### 10.4 `cn()` for Conditional Classes
