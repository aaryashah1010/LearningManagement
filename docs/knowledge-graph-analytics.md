...
1. Admin/teacher uploads the NCERT PDF for a subject's textbook (once per book, reused across every class and every test that draws on that subject). The raw file is never sent to the AI directly — a textbook PDF can be 60–200MB, almost entirely images, fonts, and formatting rather than text, and no LLM call takes a file like that as input. The upload is streamed to a temp file as it arrives (never fully buffered in application memory) — whatever a permanent-storage decision ends up being (see `database-design.md` § 4, still undecided), the temp file used for *processing* is separate from that and gets cleaned up automatically once extraction is done. It's processed in stages instead:
...
**Minimum data requirement.** A node isn't declared weak off a single question — accuracy on one question is noise, not evidence. Below a configurable minimum question count (per node, across however many tests have touched it), the report shows **"Insufficient Data"** instead of a confident weak/strong verdict. Above that minimum, configurable thresholds classify the node (e.g. 80–100% Strong, 60–79% Needs Practice, below 60% Weak) — same spirit as `../omr-grading.md` §22, applied at whatever the current graph's finest level is (Subtopic today; see the still-open depth/domain questions above for whether that changes).

**Historical performance.** Because results persist and accumulate across every test a student takes in a subject (already established in [Data Model](#data-model--results-live-separately-from-the-graph) above), a report can show a node's accuracy **per test over time**, not just a single current snapshot — e.g. Test 1: 35%, Test 2: 48%, Test 3: 61%. This is the natural way to answer "did re-teaching this topic actually work," and needs no new data — it's the same rollup query, just grouped by test instead of collapsed across all of them. Matches `../omr-grading.md` §23.
...
- **Domain grouping above Chapter (e.g. "Geometry" spanning the Circles, Triangles, and Coordinate Geometry chapters) — raised by a teammate's curriculum hierarchy (Subject → Topic → Subtopic → Concept), not yet resolved.** This isn't really "should the graph be one level deeper" — their "Topic" (the domain) sits *above* our Chapter, and their "Subtopic" is an actual chapter, so it's a different axis than the Chapter/Topic/Subtopic depth question above. Two open sub-questions:
  - **Is it a real, structural grouping** (something a report rolls up into, something a question can be scoped by), **or just a descriptive label** ("this chapter happens to be about Geometry") with no other system depending on it? The two need very different implementations — see below.
  - **If real:** unlike chapter classification, this can't be decided from one chapter's own text — it requires knowing the full set of a subject's chapters together, which is a fundamentally different, cross-chapter AI step, not an extension of the existing per-chapter pipeline (`§ Building the Graph` above). It would run **once per subject** (not per book, not per chapter), after enough chapters are already confirmed: feed the AI just the names + short summaries of every confirmed Chapter node in that subject, have it assign each one to a domain (from a fixed list, if a syllabus defines one, or proposed freely otherwise), then the same admin-review-and-confirm pattern as everywhere else. Cheap either way (one small call per subject, re-runnable as new chapters are added) — the real cost difference is in storage, not the AI call:
    - **Lightweight (recommended):** a `domain` tag/column directly on existing Chapter-level `graph_nodes` rows — no schema depth change, no re-parenting, reports can filter/group by it without it being a real tree level.
    - **Structural:** the `level` enum gains a `domain` value and every existing Chapter row gets re-parented under a new Domain node — a real migration on data that already exists, not just a new field; only worth it if reports genuinely need to roll scores up through a domain the way they roll up through Chapter/Topic/Subtopic today.

  Leaning toward the lightweight tag version unless there's a concrete reporting need for real rollup through domains — but this depends on what your teammate actually needs it for, not decided yet.
