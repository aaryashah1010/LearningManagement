...
export function FormField({
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div
        className={`relative flex items-end gap-2 border-b-2 pb-2 transition-colors focus-within:border-correct ${
          error ? "border-mark" : "border-ink/15 dark:border-paper/20"
        }`}
      >
        <input
          ref={inputRef}
      ...
      {error ? (
        <p className="font-utility text-[11px] leading-relaxed text-mark">{error}</p>
      ) : (
        helper && (
