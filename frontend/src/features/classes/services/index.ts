...
export async function fetchClasses(cursor: number | null, limit: number, search?: string) {
  const params = new URLSearchParams({ limit: String(limit) });
  if (cursor) params.set("cursor", String(cursor));
  if (search) params.set("search", search);
  const result = await apiClient.getPaginated<Class>(`/classes/?${params}`);
...
export async function fetchEnrollments(
  ...
  if (cursor) params.set("cursor", String(cursor));
  if (search) params.set("search", search);
  const result = await apiClient.getPaginated<StudentView>(`/classes/${classId}/enrollments?${params}`);
