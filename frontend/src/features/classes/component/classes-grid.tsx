...
export function ClassesGrid({ canCreate = true, basePath }: ClassesGridProps) {
  ...
  const { classes, hasNextPage, isFetchingNextPage, fetchNextPage, isLoading } = useClasses(search);
      ...
      {isLoading ? (
        ...
        <>
          ...
          {hasNextPage && isFetchingNextPage && (
            <div className="py-2 text-center font-utility text-xs text-ink/40 dark:text-paper/40">
              Loading more…
            </div>
          )}
        </>
