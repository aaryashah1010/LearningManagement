...
export function TrendChart({ title, periodLabel, unitLabel, data }: TrendChartProps) {
  ...
  function xAt(index: number) {
    return data.length > 1 ? PAD_LEFT + (PLOT_WIDTH * index) / (data.length - 1) : PAD_LEFT + PLOT_WIDTH / 2;
  }
    ...
    <div className="rounded-2xl border border-ink/10 bg-paper p-6 dark:border-paper/10 dark:bg-slate">
      ...
      <div className="relative mt-4 [--chart-surface:var(--color-paper)] [--chart-grid:color-mix(in_srgb,var(--color-ink)_8%,transparent)] dark:[--chart-grid:color-mix(in_srgb,var(--color-paper)_10%,transparent)] dark:[--chart-surface:var(--color-slate)]">
        <svg
          ...
          className="w-full"
          role="img"
          aria-label={`${title}: line chart from ${data[0]?.label} to ${last?.label}, ending at ${formatValue(last?.value ?? 0)} ${unitLabel}`}
        >
          <defs>
        ...
        {active && (
          ...
          >
            ...
            <p className="font-body text-sm font-semibold text-ink dark:text-paper">
              {formatValue(active.value)} <span className="font-normal text-ink/50 dark:text-paper/50">{unitLabel}</span>
            </p>
          </div>
        )}
      </div>
